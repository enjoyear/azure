...
public class AdfPipelineExecutioner {
  ...
  public static void main(String[] args)
    ...
    String sp = "spi-conf-cdp-adfdeployer-ei";
...
//      HttpUriRequest request = executioner.executePipeline(token, "spark-only3");
   ...
   */
  private HttpUriRequest executePipeline(AuthenticationResult token, String pipelineName)
    ...
    URIBuilder pipelineExecutionBuilder =
        new URIBuilder(buildPipelineActionUri(String.format("pipelines/%s/createRun", pipelineName)));
   ...
   * @throws IOException
   */
  private HttpUriRequest getPipelineRunStatus(AuthenticationResult token, String pipelineRunId)
      throws URISyntaxException, IOException {
    URIBuilder runMonitorBuilder =
        new URIBuilder(buildPipelineActionUri(String.format("pipelineruns/%s", pipelineRunId)));
  ...
  private HttpUriRequest getPipelineActivityRuns(AuthenticationResult token, String pipelineRunId)
      throws URISyntaxException, IOException {
    URIBuilder runMonitorBuilder =
        new URIBuilder(buildPipelineActionUri(String.format("pipelineruns/%s/queryActivityruns", pipelineRunId)));
    System.out.println("Built URI: " + runMonitorBuilder.toString());
    URI uri = runMonitorBuilder.build();
   ...
   */
  private HttpUriRequest requestHDInsightInfo(AuthenticationResult token, String hdinsightName)
      throws URISyntaxException, IOException {
   ...
   */
  private HttpUriRequest queryPipelineExecutions(AuthenticationResult token)
      throws URISyntaxException, IOException {
    RunFilterParameters parameters = new RunFilterParameters();

//    parameters.setLastUpdatedAfter("2020-03-19T09:00:00.0000000Z");
...
//            RunFilterParameters.RunQueryFilter.RunQueryFilterOperator.Equals, new String[]{"spark-only"});
//    parameters.setFilters(new RunFilterParameters.RunQueryFilter[]{filter});
  ...
  private HttpUriRequest postRequest(String bodyJson, AuthenticationResult token, URI uri)
      throws IOException {
  ...
  private HttpUriRequest getRequest(AuthenticationResult token, URI uri)
      throws IOException {
